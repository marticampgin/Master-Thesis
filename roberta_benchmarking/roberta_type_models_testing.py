...
# Mappings for RoBERTa-base and RoBERTa-twitter (identical)
...
def eval(pipe, model, model_name, test_ds, print_report=False, return_pred_info=False, train_samples=-99):
    ...
    if print_report:
        if train_samples != -99:  # arbitrary number
...
def load_data(train_path, 
    ...
    """
    This function is widely utilized in our other experiments.
    Including this file in the same folder where the .py / .ipynb
    files with desired experiment-code are located is neccessary. 
    """
    ...
    if few_shot_samples > 0 and few_shot_samples < 204:  
    ...
    if save_train_set:
        train_set = {"train": train_ds}
        pickle.dump(train_set, open(f"train_set_sample_num={few_shot_samples}_run_num={run_number}.pkl", "wb"))
...
def main():
    ...
    MAX_LEN = tokenizer.max_model_input_sizes['roberta-base']  # 512, also true for FLAN-T5
    ...
    if few_shot_samples in (0, 204):
        ...
        prediction_information = eval(pipe, 
                                      model, 
                                      model_name, 
                                      test_ds, 
                                      print_report=True,  
                                      return_pred_info=True, 
                                      train_samples=few_shot_samples)
        
        true_labels = prediction_information["true_labels"]
        predicted_labels = prediction_information["predicted_labels"]
        confidence_scores = prediction_information["confidence_scores"]

        # conf = []
        # for i in range(len(true_labels)):
        #    if true_labels[i] == predicted_labels[i]:
        #        conf.append(confidence_scores[i])

        #print(conf)        
        
        #with open(f"{model_name}_{few_shot_samples}.txt", "w", encoding="utf8") as f:
        #    texts = prediction_information["texts"]
        #    true_labels = prediction_information["true_labels"]
        #    predicted_labels = prediction_information["predicted_labels"]
        #    confidence_scores = prediction_information["confidence_scores"]
        #    for i in range(len(texts)):
        #        f.write(f"pred: {predicted_labels[i]}, gold: {true_labels[i]}, score: {confidence_scores[i]}\n")
        #        f.write(f"{texts[i]}\n\n")
